...
DOCX_MAGIC_BYTES = b"PK\x03\x04"  # Also used for PPTX/XLSX (all ZIP-based)
ALLOWED_EXTENSIONS = frozenset({".pdf", ".md", ".docx", ".pptx", ".xlsx"})
...
async def validate_and_save_pptx(upload: UploadFile, dest: Path) -> None:
    """
    Validate an uploaded file is a real PPTX and save it to disk with size limits.

    PPTX files are ZIP archives, so we check for the ZIP magic bytes (PK\\x03\\x04).
    Reads in chunks to avoid loading entire file into memory at once.
    ...
    Raises:
        HTTPException: If the file is too large, not a PPTX, or empty
    """
    total_size = 0
    first_chunk = True
    chunk_size = 1024 * 1024  # 1 MB chunks
        ...
        while True:
            ...
            if first_chunk:
                if not chunk[:4].startswith(DOCX_MAGIC_BYTES):
                    error = HTTPException(
                        status_code=400,
                        detail=f"File '{upload.filename}' is not a valid PPTX",
                    )
                    break
                first_chunk = False
...
async def validate_and_save_xlsx(upload: UploadFile, dest: Path) -> None:
    """
    Validate an uploaded file is a real XLSX and save it to disk with size limits.

    XLSX files are ZIP archives, so we check for the ZIP magic bytes (PK\\x03\\x04).
    Reads in chunks to avoid loading entire file into memory at once.
    ...
    Raises:
        HTTPException: If the file is too large, not an XLSX, or empty
    """
    total_size = 0
    first_chunk = True
    chunk_size = 1024 * 1024  # 1 MB chunks
        ...
        while True:
            ...
            if first_chunk:
                if not chunk[:4].startswith(DOCX_MAGIC_BYTES):
                    error = HTTPException(
                        status_code=400,
                        detail=f"File '{upload.filename}' is not a valid XLSX",
                    )
                    break
                first_chunk = False
